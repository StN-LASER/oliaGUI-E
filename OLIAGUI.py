...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.samplesPperiodBox.activated.connect(self.samplesPperiodChanged)        # Nikos Extended3
        ...
        try:
            if self.measureButton.isChecked() == 0:
                ...
                while self.measureButton.isChecked() == 1:
                    ...
                    self.samplesInPeriod.setText(str(float(measurements[5])))    # Nikos: Experimental new box in GUI, reads new Parameter send from the FW: samplesPerperiod [5] or [18]
                    self.main_aux.setText(str(float(measurements[19])))          # Nikos: Experimental new box in GUI, reads new Parameter send from the FW: 0/1 = main/aux
                    self.samplingFrequency.setText(str(float(measurements[18]))) # Nikos: Experimental new box in GUI, reads new Parameter send from the FW: 0/1 = main/aux
    ...
    def filterToggled(self):
        ...
        if checking[3] == '0':
            self.dialogueBox.setText("Synchronous filter REMOVED IN THIS VERSION")
    ...
    def samplesPperiodChanged(self):    # Nikos Extended3
        inSPP = self.samplesPperiodBox.currentText() #  Nikos Extended3 samplesPerPeriod from drop menu
        SPPtring = 'p' + inSPP #time constant commands prefixed with an 'e'
        com.sendCommand(self.port, SPPtring) # Nikos Extended3 send the value with new prefix 'p' to change samplesPerPeriod
        self.dialogueBox.setText("Extended3:Only works in aux mode: Samples Per Period changed, affecting Sampling Frequency")
        app.processEvents()        
